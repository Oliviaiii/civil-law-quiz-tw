...
for (const year of A.years) {
  for (let n = A.numFrom; n <= A.numTo; n += 1) {
    const nn = String(n).padStart(2, '0')
    questions.push({ id: `judicial-fourth-${year}-legal-knowledge-${nn}`, rocYear: year })
  }
}
log(`${subjectLabel}：共 ${questions.length} 題（${A.years.join('、')} 年）`)
