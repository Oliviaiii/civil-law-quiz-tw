...
## 2026-07-23 民法 175 題逐選項複核完成

- 司法四等民法 108～114 年共 175 題選擇題，已全部補成結構化 `application: { intro?, A, B, C, D }`；合計 700 個選項理由，實際解析畫面會逐項分開呈現。
- 複核資料位於 `app/data/analyses/civil-option-reviews-{108..114}.json`，並由 `app/data/judicial-fourth-analyses.ts` 覆蓋舊版只說明答案的字串；缺少舊種子或逐選項複核時，不再產生該題解析。
- 官方答案以題庫每題的考選部 `answerUrl` 為準；113 年第 24 題依更正清冊維持一律給分。法條依據沿用並補強各題 `articles`，連回法務部全國法規資料庫；官方題目來源仍保留在每題 `sourceUrl`。
- `tests/rendered-html.test.mjs` 新增嚴格防線：必須恰有 175 題、題號與正式題庫完全相同、每題恰有 A～D 四欄、每欄至少 20 字、700 段不得重複或使用泛用模板，且每題須有考選部試題及答案網址。
- `tests/e2e/practice.spec.ts` 新增實際畫面檢查，確認民法 A～D 四段理由都會顯示。
- 已通過 `npm run lint`、`npm test`、`npx playwright test --workers=1`（47／47）；本輪沒有 commit、push 或部署。

## 2026-07-22 完成狀態

- 原「解析整理中」43 題已全部完成：法學緒論 113 年第 18–30 題、憲法 114 年第 1–15 題、法學緒論 114 年第 16–30 題。
- 憲法解析為 150／150，法學緒論解析為 150／150，合計 300／300。
- 每題均有 `issue`、`rule`、四選項逐項 `application`、`trap`、信心標記及至少一筆參考來源；研究來源另記錄於 `docs/handover/sources/`。
- `docs/handover/remaining-batches.json` 已清空為 `[]`。
- `tests/rendered-html.test.mjs` 已恢復嚴格完整性檢查：兩科各少於 150 題或任一題缺少人工解析時，測試會失敗。
- 已重新產生題庫與靜態資料，並通過 `npm run lint`、`npm test`、`npx playwright test --workers=1`（45／45）；本輪沒有執行部署。
...
4. 刑法 174 題重寫 → ~~民法 175 題重寫（2026-07-23 完成）~~ → 英文 200 題補寫理由 → 國文 100 題人工複核格式。
