export const meta = {
  ...
  phases: [
    { title: 'Verify', detail: '逐題核對既有研究稿的答案一致性、法條與品質' },
...
const results = await pipeline(
  questions,
  (q) => agent(verifyPrompt(q), { label: `verify:${q.id.replace(/^judicial-fourth-/, '')}`, phase: 'Verify', schema: VERIFY_SCHEMA }),
  async (ver, q) => {
    if (ver && ver.verdict !== 'failed') return ver
    const notes = ver ? ver.issues : '驗證代理未完成。'
