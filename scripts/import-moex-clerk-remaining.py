...
def normalize(text: str) -> str:
    ...
    text = re.sub(r"(?m)^.*公務人員特種考試.*$", "", text)
    text = re.sub(r"(?m)^.*調查局調查人員.*$", "", text)
    text = re.sub(r"(?m)^.*考試試題.*$", "", text)
    text = re.sub(r"(?m)^\s*(?:考\s*試\s*別|等\s*別|類\s*科\s*組|科\s*目)\s*[：:].*$", "", text)
    text = re.sub(r"(?m)^\s*[（(]?(?:正面|背面)[）)]?\s*$", "", text)
    text = re.sub(r"(?m)^代號：.*$", "", text)
    text = re.sub(r"(?m)^頁次：.*$", "", text)
    text = re.sub(r"(?m)^\s*[\d、，－-]{5,}(?:\s+[\d、，－-]+)*\s*$", "", text)
    text = re.sub(r"(?m)^\s*[−-]\s*$", "", text)
    text = re.sub(r"(?s)\s*(?:\d{5}(?:[-－]\d{5})?\s*)?全一張.*$", "", text)
    text = re.sub(r"(?m)^\s*（?請接背面）?\s*$", "", text)
    text = re.sub(r"\s+", " ", text)
    text = re.sub(r"\s+(?:[2-9]|10)[.．]\s*$", "", text)
